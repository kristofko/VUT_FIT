...
void untilLeftPar( Stack *stack, char *postfixExpression, unsigned *postfixExpressionLength ) {
    ...
    // Get rid of the left parenthesis in stack
    if ( temp == '(' ){
        Stack_Pop(stack);
    }
    return;
...
void doOperation( Stack *stack, char c, char *postfixExpression, unsigned *postfixExpressionLength ) {
    ...
    while(1){
        ...
        // Top of stack is left parenthesis
        Stack_Top(stack, &temp);
        if (temp == '(') {
            Stack_Push(stack,c);
            break;;
        ...
        // top of stack is higher priority operator
        else if (c == '*' || c == '/') {
            if ( temp == '+' || temp == '-'){
                Stack_Push(stack, c);
                break; 
            }
            // top of stack is the same priority (for '*', '/')
            else {
                Stack_Pop(stack);
                postfixExpression[*postfixExpressionLength] = temp;
                *postfixExpressionLength = *postfixExpressionLength + 1;    
...
char *infix2postfix( const char *infixExpression ) {
    // string to write postfixExpression to
    char *postfixExpression = malloc (MAX_LEN);
    if (postfixExpression == NULL){
        fprintf(stderr,"Intern malloc problem, not enough space for postfix expression");
        return NULL;
    }
    ...
    for (int i = 0; infixExpression[i] != '\0'; i++){
        ...
        if ( (c == '+') || (c == '-') || (c == '*') || (c == '/') ){
            // characeter is operator
            doOperation(stack , c, postfixExpression, &postfixIndex);
        }
        
        // After '=' is found, empty stack and return postfix Expression
        if ( c == '=' ){
            while(!Stack_IsEmpty(stack)){
                Stack_Top(stack,&temp);
                Stack_Pop(stack);
                if (temp != '('){
                    postfixExpression[postfixIndex] = temp;
                    postfixIndex++;
                }
            }
        }
